...
\subsection{Newtonian derivation of Friedmann Equation}
...
\begin{thm}[Newton's shell theorem]
	\begin{enumerate}
...
NB. Real motion of galaxies is made up of flow plus peculiar motion.

There are two directions we want to go in.
\begin{enumerate}
	\item What if we replace matter with photons? (Early universe).
	\item What if energy is non-zero?
\end{enumerate}
...
\begin{align*}
	 ...
	 \frac{\dot{a}}{a}^2 - \frac{2MG}{a^3} =& \frac{C}{a^2} \\
	 \frac{\dot{a}}{a}^2 - \frac{8\pi\rho G}{3} =& \frac{C}{a^2}\\
	 \frac{\dot{a}}{a}^2  =& \frac{8\pi\nu G}{a^3} + \frac{C}{a^2} \numberthis \label{eq:full:friedmann}
\end{align*}

\eqref{eq:full:friedmann} is the full Friedmann equation, which he derived from General Relativity; we have derived from Newton.

if $C>0$, right hand side is strictly positive, universe continues to grow forever (For if $\dot{a}>0$ initially, it has to pass through zero to stop expanding). If $a$ small enough $\frac{8\pi\nu G}{a^3}$ will dominate, and we can use \eqref{eq:2:3}. Once $a$ grows enough $ \frac{C}{a^2}$ dominates.

\begin{align*}
	\frac{\dot{a}}{a}^2  =&  \frac{C}{a^2} \\
...
\begin{align*}
	\underbrace{\frac{\dot{a}}{a}^2  - \frac{\kappa}{a^2}}_\text{Terms involving geometry}=& \underbrace{\frac{8\pi}{3} G \rho}_\text{Matter and energy} \numberthis \label{eq:geometry:energy}
\end{align*}
We'll replace mass density of energy density. Energy = $Mc^2 + $ Radiation (and we will choose units such that $c=1$).
...
\begin{figure}[H]
	...
	\begin{center}
			...
			\begin{subfigure}[t]{0.45\textwidth}
				\begin{center}
					\caption{An analogy with standing waves. Number of nodes (zero crossings) is an adiabatic invariant:  it has to be an integer.}
...
We have to do some more quantum mechanics of classical electromagnetism to justify the statement in Figure \ref{fig:unit:box}. For the time being, take it as a given. So energy in box decreases: $E\propto \frac{1}{a}$ an.d $\rho\propto \frac{1}{a^4}$.

\begin{align*}
	\frac{\dot{a}}{a}^2  =& \frac{8\pi\nu G}{a^4} - \frac{C}{a^2} \text{, setting $C=0$}\\
	\frac{\dot{a}}{a}^2  =& \frac{8\pi\nu G}{a^4} \text{, or, with suitable units}\\
	\frac{\dot{a}}{a}^2  =& \frac{1}{a^4} \text{, taking square root and rearranging} \\
	\dot{a} =& \frac{1}{a}\\
	\frac{dt}{da} =& a\\
	t =& a^2{, in suitable units}\\
	a =& \sqrt{t} 
\end{align*}

\begin{figure}[H]
	\caption{Comparison of radiation and matter dominated universes}\label{fig:cosmo-2-a-r}
	\begin{center}
