...
\begin{align*}
	ds^2 =& - dt^2 + a(t)^2 d\Omega_3^2 &\kappa=1\text{, or}\\
	ds^2 =& - dt^2 + a(t)^2 \big(dx^2 + dy^2 +dz^2\big) &\kappa=0\text{, or}\\
	ds^2 =& - dt^2 + a(t)^2 d\mathcal{H}_3^2 &\kappa=-1	\text{ }
\end{align*}
In all three cases:
\begin{align*}
	D =& a(t) d\theta\\
...
\begin{align*}
	\big(\frac{\dot{a}}{a}\big)^2 + \frac{\kappa}{a^2}=& \frac{8\pi}{3} \rho \text{, or}\\
	\big(\frac{\dot{a}}{a}\big)^2 =& \frac{8\pi G}{3}\rho - \frac{\kappa}{a^2} \text{, c.f. Equation \eqref{eq:geometry:energy}} \numberthis \label{eq:einstein:equation}
\end{align*}
...
$\Lambda$ is known as the ''cosmological constant'', or as ''dark energy''. For the significance of the factor $\frac{3}{8 \pi G}$, see \eqref{eq:full:friedmann}
...
Let us study the case where the vacuum energy density is the \emph{only} density. Earlier we look at a pure matter dominated universe, Section \ref{sec:matter:dominated}, and now we'll look at a pure vacuum energy universe. There are six cases for non-zero $\Lambda$ and  for $\kappa$--Table \ref{table eq:options:lambda:kappa}.
...
\begin{table}[H]
	\begin{center}
		\caption{Options for $\Lambda$ and $\kappa$}\label{table eq:options:lambda:kappa}
...
Substituting  $\Lambda>0$ and $\kappa=0$ in Friedmann's equation \eqref{eq:friedmann:avec:lambda} gives:
\begin{align*}
...
\end{align*}
 ...
 Substituting $\Lambda>0$ \& $\kappa>0$ in the Friedmann equation, \eqref{eq:full:friedmann}, gives:
 \begin{align*}
 	\big(\frac{\dot{a}}{a}\big)^2  =& \Lambda - \frac{1}{a^2}\\
...
\begin{align*}
	...
	\xi(r)=&\sin{r}& \text{ if } \kappa = +1\\
	\xi(r)=&\sinh{r}& \text{ if }\kappa = -1\\
	\xi(r)=&r& \text{ if } \kappa = 0
\end{align*} $\xi$ depends on the geometry.

There are only 3 contributions to $\rho$ known.

\begin{align*}
	- \frac{\kappa}{a^2} + \frac{8 \pi G}{3}\rho = \underbrace{\frac{C_R}{a^4}}_\text{Radiation} + \underbrace{\frac{C_M}{a^3}}_\text{Matter}+ \underbrace{\Lambda}_\text{Vacuum energy} - \frac{\kappa}{a^2}\\
...
Today we call the ratios $\Omega_R$, $\Omega_m$, $\Omega_{\Lambda}$, and $\Omega_{\kappa}$,normalized so:
\begin{align*}
	\Omega_R+\Omega_m+\Omega_{\Lambda}+\Omega_{\kappa}=&1
\end{align*}

When LS was a student, $\Omega_R$ was negligible, $\Omega_{\Lambda}$ was unknown, and Hubble constant was 20 times larger than $\Omega_m$, so it was assumed that $\Omega_{\kappa}\approx 1$. The meant on open, infinite universe with $k\approx -1$. For most of the time $\frac{c_M}{a^3}$ competes favorably with $\frac{\kappa}{a^2}$
...
\begin{align*}
	\frac{C_M}{a^3} - \frac{\kappa}{a^3} = H^2
\end{align*}

A model consists of $\kappa$ and an equadrtion of state, for which we can substitute a history, $a(t)$. We can measure red-shift, and luminosity.

To start, assume we can ignore time. how can we measure $\kappa$? Meausure number of galaxies of a given brightness, as a function of distance.
...
Let $\lambda_{e}mitted$ denote the wavelength of light is emitted from a source. This may or may not be the wavelength that is detected: Doppler shift is one possible reason; gravitational shift due to a black hole is another; expansion of the universe is equivalent to the Doppler shift, but it is what we are interested in.
...
\begin{defn}[Redshift]
	\begin{align*}
		z =& \frac{\lambda_{Detected}}{\lambda_{Emitted}}-1 \text{, $-1$ for historical reasons}\\ 
...
There is nothing we can do with this until we make a model. We will take the matter dominated universe, and set $\kappa=-1$.
...
\begin{itemize}
	...
	\item long in the future $\Omega_{\Lambda}\approx1$
\end{itemize} 
...
The universe appears to be electrically neutral: $N_{e^-}=N_P$. Hydrogen atoms aren't effective scatterers, so today the universe is not in thermal equilibrium, or the different kinds of particles aren't in thermal equilibrium with each other. If electrons and protons weren't bound, they would be efficient scatterers, and the universe would have a well defined temperature. We need a high temperature to ensure they aren't bound.

Returning to our box, we have a temperature $T$, and each photon has an energy $\lambda$ and frequency $\nu$ (there are many different wavelengths in the box).

Radiation characterized by the intensity: $I(t,\nu)dVd\nu$ is energy in band. First attempt (before 1900), using dimensional analysis.
\begin{align*}
	dim \; I(t,\nu) =& \big[\frac{E T}{V}\big]
\end{align*}

What are units of temperature? (energy per particle) $E = k_BT$--Boltzmann's constant is just a conversion factor: temperature can be treated as energy. We can ask what sort of dependence $I$ can have on $T$ and $\nu$? The only thing that will work dimensionally is:
...
This a a disaster: $I$ grows without bound an $\nu$ grows, and the energy in the box is infinite. We need another consonant to avoid the ''ultraviolet catastrophe''. The correct formula for black body radiation is:
 
\begin{align*}
	I =& \frac{2 h \nu^3}{c^2} \frac{1}{e^\frac{h\nu}{k_BT}-1} \numberthis \label{eq:planck:black:body}
\end{align*}

For small $\nu$, \eqref{eq:planck:black:body} reduces to \eqref{eq:raleigh:jeans}, and for  $\frac{h\nu}{kT} \gg 1$ it behaves more like $e^\frac{-h\nu}{k_BT}$, averting the ultraviolet catastrophe.

At the crossover point
\begin{align*}
	\frac{h\nu}{kT} =& 1 \text{, or}\\
	\frac{hc}{kT} =& \lambda \text{, the thermal wavelength associated with that temperature.}
\end{align*}
...
\begin{align*}
	I \propto & \underbrace{\frac{\nu^3}{T^3} \frac{1}{e^\frac{h\nu}{k_BT}-1}}_\text{Determines shape of curve} T^3
\end{align*}

If we change temperature, $\frac{\nu^3}{T^3} \frac{1}{e^\frac{h\nu}{k_BT}-1}$ simply rescales. This is the universal behaviour of black body radiation.
...
When temperature is too low in the box, the radiation decouples from the charged particles.

Imagine box starts at a high temperature so matter is ionized. As we expand the box it cools, temperature goes down, and particles combine into atoms. There isn't enough oomph to ionize atoms, ad the radiation decouples. The temperature of the radiation goes down, but it doesn't stay in thermal equilibrium. As box expands, the shape of the black body radiation is frozen in. Wavelengths are stretches, which is equivalent to changing temperature, but only for radiation (Molecules have a different temperature). That is why radiation today has black body form (this has been measured). \gls{gls:cmb} has been studied, and error bars are very tiny. We see universe filled with radiation that appears to be the remnant of a thermal distribution that must have been thermalized at a very high temperature when all matter was ionized.

\begin{align*}
	\frac{\lambda_{TODAY}}{\lambda_{DECOUPLING}} =& \frac{a(TODAY)}{a(DECOUPLING)}\\
	=&\frac{T_DECOUPLING}{T_{TODAY}} \text{, \gls{gls:cmb}}\\
	\approx& \frac{T_{ionization}}{3K} \text{, ionization energy for hydrogen=13.5 ev}
\end{align*}

The ratio of N(photons) to N(electrons) is very large: $\frac{N_\gamma}{N_e}\approx 10^8$. (Observable fact: why?)


The probability of a photon having energy $\epsilon$ is $e^{-\frac{\epsilon}{kT}}$. What is probability that it will have the ionization energy is $e^{-\frac{\epsilon_{ion}}{kT}}$. If $\epsilon>kT$ this is very small, but there are many photons. We could ask how many photons can ionize the atom? $10^8 e^{-\frac{\epsilon_{ion}}{kT}}$. We are operating in tail of Boltzmann distribution. If $10^8 e^{-\frac{\epsilon_{ion}}{kT}} \approx 1$, there are enough photons to ionize the electrons. $10^8 e^{-\frac{\epsilon_{ion}}{kT}} \approx e^20 e^{-\frac{\epsilon_{ion}}{kT}}$
...
Frst, we need the ratios today.
\begin{align*}
	\rho_{matter} =& \frac{p_m}{a^3} \text{, most of observable history}\\
	\rho_{radiation}=& \frac{p_4}{a^4} \text{. Let us take the energy density today}\\
...
In flat space, only ratios of $a$ matter. If $\kappa>0$, we can make sense of $a(t)$ as the radius of the 3-sphere.  If $\kappa<0$, the universe started infinite.

\begin{figure}[H]
	\caption{The universe in a nutshell}
	\begin{subfigure}[t]{0.45\textwidth}
		\caption{Looking backward aolong light rays}
...
When temperature gets to about $10^{14}K$, $\frac{a_T}{a(t)}\approx 10^{14}$, characteristic photon energy $\epsilon_{photon}=0.5MeV$,  the $mc^2$ of an electron. Two photons colliding can produce electron-positron pairs; also electrons and positrons can make photons. They can form a thermal equilibrium, and a no longer determined by ''memory of the future''. Thermal equilibrium at that temperature satisfies $n_{e^-}=n_{e^+}=n_{\gamma}$. Note that  $n_{e^-}-n_{e^+}$ does not change, but $n_{e^-}+n_{e^+}$ does: it is bigger than today's value by $10^8$.

What led to any difference?



\url{https://youtu.be/GvhxamzVpN8?t=5261}
