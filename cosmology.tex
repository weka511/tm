...
\begin{figure}[H]
	...
	\end{subfigure}
	\begin{subfigure}[t]{0.47\textwidth}
		\caption{Proton decay}\label{fig:cosmo-8-proton-decay-mechanism}
		\includegraphics[width=0.6\textwidth]{cosmo-8-proton-decay-mechanism}
	\end{subfigure}	
\end{figure}

For some reason, that Standard Model does not permit the decal in Figure \ref{fig:cosmo-8-proton-decay}, but there are extensions to the Standard Model where this could happen. We do know that the decay cannot be very fast, as the protons have been around for at least 13.7 billion years. So we think that the excess has been built in from the very beginning.
...
\subsection{The Modern Theory}
 This begins with the idea that there \emph{was} a balance: that there was no bias towards particles or antiparticles. The only way to get unbalanced is if conservation of \gls{gls:baryon:number} is not correct. If something like Figure \ref{fig:cosmo-8-proton-decay} can happen, the number of protons can change. The first requirement for a theory that starts with balace is to have a mechanism that violates symmetry. That was the first of the Sakharov conditions.
 \begin{enumerate}
 	\item \gls{gls:baryon:number} violation, e.g. Figure Figure \ref{fig:cosmo-8-proton-decay}. If \gls{gls:baryon:number} conservation is not a good symmetry, it must be very weakly broken, and have a very small probability. Protons are very, very old. Every known unified theory violates \gls{gls:baryon:number} conservation.  You might ask why, in the known theories, the proton is so stable. Somewhere in the guts of the Feynman diagram, Figure \ref{fig:cosmo-8-proton-decay-mechanism}, assume there is a very, very heavy particle, outside the Standard Model, say $10^{16}GeV$, then the process is very slow, and is suppressed by powers of the heavy mass. If the proton has extra energy $\frac{1}{M^2}$ becomes $\frac{1}{(M-E)^2}$. Very hot at beginning! But this isn't enough: the process could decay antiprotons, or run in reverse. The excess cannot be statistical effect: we'd expect $\sqrt{10^{80}}=\sqrt{10^{40}}$. Also why the same everywhere? We need bias. 
 	\item We need violations of Particle-Antiparticle symmetry, and both C and CP  symmetry. We have experimental evidence: the $B$ and $\bar{B}$ mesons. The rates of decay are grossly different.
 	\item We have CPT symmetry in all know theories. In thermal equilibrium backward and forward time are the same; so we have T symmetry, so CPT symmetry gives CP symmetry. But expanding universe not in equilibrium. In a rapidly expanding universe, T symmetry is broken. Must be fast! But then suddenly slowed.
 \end{enumerate}
 
 To summarize:
 \begin{enumerate}
 	\item \gls{gls:baryon:number} violation
 	\item We need violations  C and CP (bias)
 	\item Out of equilibrium. (T symmetry broken)
 \end{enumerate}
