...
\begin{enumerate}
	...
	\item Universe is expanding enough to break T symmetry--Section \ref{sec:sakharov3}
\end{enumerate}
...
\end{itemize}
 ...
 \begin{figure}[H]
 	\begin{center}
 		...
 		\includegraphics[width=0.8\textwidth]{cosmo-8-muse}
 	\end{center}
 \end{figure}
...
We want the equation of motion.

\begin{align*}
	\mathcal{L} =&  a^3(t)\big[\frac{\dot{\phi}^2}{2} - V(\phi)\big]\\
	\frac{d}{dt} \frac{\partial \mathcal{L}}{\partial \dot{phi}}=& \frac{\partial \mathcal{L}}{\partial \phi}\\
	\frac{d}{dt} \frac{\partial \mathcal{L}}{\partial \dot{\phi}}=&\frac{d}{dt}\big[\dot{\phi}a^3(t)\big]	\\
	\frac{d}{dt}\big[\dot{\phi}a^3(t)\big]=&-a^3(t)\frac{dV}{d\phi}\\
	a^3\ddot{\phi} + \dot{\phi}\big(3a^2\dot{a}\big)=&a^3F \text {, ''The Force''}\\
	\ddot{\phi} + 3\frac{\dot{a}}{a}\dot{\phi}=&F\\
	\ddot{\phi} + \underbrace{3H\dot{\phi}}_\text{Cosmic friction term}=&F
\end{align*}
This is the falling stone, Equation \eqref{eq:falling:stone}, with a viscous drag proportional to $H$: Hubble acts as friction.
