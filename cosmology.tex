...
\begin{itemize}
	...
	\item A long pointy ellipsoid is not homogeneous. You woiuld notice the difference if you were walking around on it.
	\item Surfaces with bumps are not homogeneous.
\end{itemize}

What kinds \emph{can} be curved and homogeneous? There are two kinds.
\subsection{Flat Space and the 3-sphere}
Let's think about a metric, and restrict to 2 dimensions.

\begin{align*}
	ds^2 =& dx^2 + dy^2\text{, ordinary flat space}\\
	=&dr^2 + r^2 d\theta^2 \text{, changing to polar coordinates. Or}\\
...
\begin{figure}[H]
	\caption[We can treat circle as a piece of string and deform it]{We can treat circle as a piece of string and deform it. Label by angular coordinate so equal distances map to the equal angles. A circle is just a curve where we come back to the same place after an angle of $2\pi$}.
	\includegraphics[width=0.8\textwidth]{cosmo-3-1-sphere}\label{fig:cosmo-3-1-sphere}
\end{figure}

In polar coordinates, it \emph{looks} as if there is a special place, but, in reality, all points are the same. think of flat space as a nested collection of circles. 
...
\begin{figure}[H]
	...
	\includegraphics[width=0.8\textwidth]{cosmo-3-galaxies-2-sphere}
\end{figure}
...
We can also count the number of galaxies: on  a sphere you see fewer galaxies at great distances, compared to the plane. At attipodes we see just one galaxy.
...
\begin{figure}[H]
	...
	\includegraphics[width=0.8\textwidth]{cosmo-3-stereographic-projection}
\end{figure}

How do circles on the plae map to circles on the sphere? If they are close to South pole they look the same. As we move further the projections get bigger, and the extreme Arctic circle looks very large. Sphere is homogeneous, but this doesn't match plane. 
...
Modern cosmology assumes that the space we live in is one of the three that we have considered (not quite true: it could be a torus). On scales that we can currently detect, the Universe appears flat--out to 22 billion light years, so we know that the radius must be at least 10 times this side.
