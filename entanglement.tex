...
\begin{proof}
	...
	\end{figure}
We use "proof by diagram"--Figure \ref{fig:bell}.
\end{proof}
...
\begin{table}[H]
	\caption{States for Classical Bell's inequality}
	\begin{center}
...
\end{align*}
Note that we can replace numbers with probabilities, since we are working with zillions of electrons. When we calculate the probabilities for the singlet state, we find that the $\ge$ sign in (\ref{eq:bell:ineq:classical}) comes out <, violating Bell's inequality.

\url{https://youtu.be/XlLsTaJn9AQ?t=2682}
