...
\begin{table}[H]
	...
	\begin{center}
		\begin{tabular}{|l|l|l|}\hline
			...
			B&$\frac{1}{2}\big(\frac{\sigma_1+\sigma_3}{\sqrt{2}}+I\big)$&$\frac{1}{2}\big(\frac{\tau_1+\tau_3}{\sqrt{2}}+I\big)$\\ \hline
			C&&$\frac{1}{2}\big(\tau_3+1\big)$\\ \hline
		\end{tabular}
	\end{center}
\end{table}
...
\begin{align*}
	...
	=&\frac{\ket{dd}}{\sqrt{2}} + \big(1+\frac{1}{\sqrt{2}}\big)\ket{du} \text{, and}
\end{align*}
Multiplying by $\sigma_3 + I$:
\begin{align*}
...
Returning to Table \ref{table:projection:bell}, $P(A, \neg C) = \frac{1}{4}\big(\sigma_3 + I\big) \big(\tau_3+1\big)$. Applying this to the Singlet, and using rotational invariance, the right side of (\ref{eq:bell:ineq:classical}) becomes:
\begin{align*}
	\frac{1}{\sqrt{2}}&\big[\bra{ud}-\bra{du}\big]\frac{1}{4}\big(\sigma_3 + I\big) \big(\tau_3+I\big)\frac{1}{\sqrt{2}}\big[\ket{ud}-\ket{du}\big]\\
	=&\frac{1}{8}\big[\bra{ud}-\bra{du}\big]\big(\sigma_3 + I\big) \big(\tau_3+I\big)\big[\ket{ud}-\ket{du}\big]
\end{align*}

\begin{align*}
	\big(\tau_3+I\big) \ket{ud} =& 0\\
	\big(\tau_3+I\big) \ket{du} =& 2\ket{du}\\
	\big(\sigma_3 + I\big) \big(\tau_3+I\big)\big[\ket{ud}-\ket{du}\big]=&
	-2 \big(\sigma_3 + I\big) \ket{du}\\
	=& 0 \text{ WTF?}
\end{align*}
LS gets 0.25 for RHS. contradicting (\ref{eq:bell:ineq:classical}). Verify calc! Singlet violates Bell's inequality, which means that there is no possibility of an underlying classical way of thinking about QM based on set theory. It can not be the statistical theory of dome underlying complicated chaotic classical system. 

Check Aspect's experiment!
