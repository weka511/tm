...
\begin{align*}
	...
	=& \partial_r \Gamma\indices{^r_{\theta\theta}} - \partial_\theta \Gamma\indices{^\phi_{\theta\phi}} + \bigg(\Gamma\indices{^t_{rt}}+\Gamma\indices{^r_{rr}}+\cancel{\Gamma\indices{^\theta_{r\theta}}}+\Gamma\indices{^\phi_{r\phi}}\bigg) \Gamma\indices{^r_{\theta\theta}} -\cancel{ \Gamma\indices{^r_{\theta\theta}} \Gamma\indices{^\theta_{\theta r}}}-\Gamma\indices{^r_{r\theta}} \Gamma\indices{^r_{\theta r}} - \Gamma\indices{^\phi_{\phi\theta}} \Gamma\indices{^\phi_{\phi\theta}}\\
	=& \partial_r \bigg[-r  e^{-\lambda(r)}\bigg] - \partial_\theta \bigg[-\tan \theta\bigg]  + \bigg(\underbrace{\frac{\nu^\prime(r)}{2}+\frac{\lambda^\prime(r)}{2}}_\text{$=0$ from (\ref{eq:lambda:nu})}+\frac{1}{r}\bigg)\bigg[ -r  e^{-\lambda(r)}\bigg]
	 -\frac{1}{r^2} - \tan^2 \theta\\
	=& -  e^{-\lambda(r)} + r \lambda^\prime(r) e^{-\lambda(r)} + \underbrace{\sec^2 \theta}_\text{$=1+\tan^2 \theta$} -e^{-\lambda(r)} -\frac{1}{r^2} - \tan^2 \theta\\
	=& -  e^{-\lambda(r)} + r \lambda^\prime(r) e^{-\lambda(r)} + 1 -e^{-\lambda(r)} -\frac{1}{r^2}  \numberthis \label{eq:RtHtH}
\end{align*}
