...
\begin{align*}
	...
	=& \partial_r \bigg[-r  e^{-\lambda(r)}\bigg] - \partial_\theta \bigg[-\tan \theta\bigg]  + \bigg(\underbrace{\frac{\nu^\prime(r)}{2}+\frac{\lambda^\prime(r)}{2}}_\text{$=0$ from (\ref{eq:lambda:nu})}-\cancel{\frac{1}{r}}+\cancel{\frac{1}{r}}\bigg)\bigg[ -r  e^{-\lambda(r)}\bigg]\\
	=& -  e^{-\lambda(r)} + r \lambda^\prime(r) e^{-\lambda(r)} + \sec^2 \theta \text{ WTF?} \numberthis \label{eq:RtHtH}
\end{align*}
