...
\begin{figure}[H]
	...
	\end{subfigure}
	\begin{subfigure}[t]{0.45\textwidth}
		\caption{Universe grows. Eventually it gets big enough for two parts to be unable to communicate, represented as a bifurcation. Colours represent bubbles. Each line is a universe in which somebody may be able to live. Colour changes represent Coleman-de Lucca bubbles.}\label{fig:wt1ws-growth}
		\includegraphics[width=\textwidth]{wt1ws-growth}
	\end{subfigure}
	\begin{subfigure}[t]{0.45\textwidth}
		\caption{Let's imagine that we move along a branch where we can live. ({eq:wt1ws:markov}) represents probabilities or seeing various things. This gives the same sort of answers as Boltzmann's box--the wrong kind of answers.}\label{fig:wt1ws-lets-imagine}
		\includegraphics[width=\textwidth]{wt1ws-lets-imagine}
	\end{subfigure}
\end{figure}

Average number of universes of type $m$
\begin{align*}
	N_m (t+1)=& 2 N_m(t)-\sum \gamma_{nm} N_{m}(t)  + \sum \gamma_{mn}N_n \numberthis \label{eq:wt1ws:markov}
\end{align*}

This is another dead end. We keep coming back to random fluctuations.

If $\lambda<0$ there are crunches, ans some bubbles have the possibility of dying.

\begin{figure}[H]
	\caption{Maybe red corresponds to a Crunch}\label{fig:wt1ws-crunches}
	\includegraphics[width=0.8\textwidth]{wt1ws-crunches}
\end{figure}

\begin{align*}
N_m (t+1)=& 2 N_m(t)-\sum \gamma_{nm} N_{m}(t)  + \sum \gamma_{mn}N_n - \gamma_m N_m
\end{align*}
This is similar to an equation with a fluid flowing from source to sink, which created eddies. Vortices have a sense of time: time is a one way street.
